...
def MonteCarlo3d(ab: np.array, N: int, f, **kwargs):
    dots = {
        "x": np.random.uniform(*ab[0], N),
        "y": np.random.uniform(*ab[1], N),
        "z": np.random.uniform(*ab[2], N)
    }

    return (abs(ab[0][0] - ab[0][1]) * abs(ab[1][0] - ab[1][1]) * abs(ab[2][0] - ab[2][1])) * np.mean(f(**dots))


if __name__ == "__main__":
    data = [
        {"ab": np.array([[0, 1], [0, 1], [0, 1]]), "N": 10 ** 4, "f": lambda x, y, z: x * y * z, "f_text": "x * y * y", "manual_result": .125},
    ]

    for parametrs in data:
        obj = MonteCarlo3d(**parametrs)
        print(
